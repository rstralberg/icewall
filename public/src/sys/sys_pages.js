...
function sys_pages_form_page_selected() {
    ...
    let pageid = query_value('sys-pages-form-pages');
    let pagelist = query_id('sys-pages-form-pagelist');
    query_id('sys-pages-form-selected').value = 'p'+pageid;
    pagelist.innerHTML = '';
    ...
    server('pages/get_pages', {
        ...
        (resolve) => {
            ...
            pages.forEach(page => {
                ...
                if( page.id  === pageid ) {
                    query_id('sys-pages-form-selected-name').value = page.title;
...
function sys_pages_form_new_name() {
    enable_element('sys-pages-form-rename-button', query_value('sys-pages-form-rename').length > 0);
}

function sys_pages_form_rename() {
    server('pages/rename_page', {
        pageid: query_value('sys-pages-form-selected').substring(1),
        title: query_value('sys-pages-form-rename')
    }).then( 
        () => {
            update_navbar();
        },
...
function sys_pages_form_parent_selected() {
    let parent = query_value('sys-pages-form-parent-select');
    let id = query_value('sys-pages-form-selected').substring(1);
...
function sys_pages_form_move_up() {
    ...
    let selected_pageid = query_value('sys-pages-form-selected');
    let ul = query_id('sys-pages-form-pagelist');
    if (ul === null)
        return;
...
function sys_pages_form_delete() {
    let selected_page = query_value('sys-pages-form-selected');
    if( is_valid(selected_page) ) {
        let pageid = selected_page.substring(1);
        yesno('Radera sida', 'Är du säker på att du vill radera sidan "'+ query_value('sys-pages-form-selected-name') + '"', 'sys_pages_form_delete_page', 'sys_pages_form_skip_delete_page');
    }
}
