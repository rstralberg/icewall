...
function dbDatabaseExist(string $dbName) : bool {
...
function dbConnect($database=null): mysqli
{
	...
	mysqli_query($myscli, 'CREATE DATABASE IF NOT EXISTS ' . ($database===null?DB_DATABASE:$database));
	$myscli->close();
	
	$myscli = mysqli_connect(DB_HOST, DB_USER, gethostname() === DEV_HOSTNAME ? DB_PASSW_DEVEL : DB_PASSW);
	if ($myscli->connect_error) {
		throw new Exception( 'Failed to open database ' . ($database===null?DB_DATABASE:$database) . ': ' . $myscli->connect_error);
	}

	mysqli_query($myscli, 'USE ' . ($database===null?DB_DATABASE:$database));
	return $myscli;
}
...
function dbAddDefaultRow(mysqli $mysqli, string $table) : int {
    mysqli_query($mysqli, 'INSERT INTO ' . sqlName($table) . '() VALUES ()' );    
    return $mysqli->insert_id;
}
...
function dbWereString( mysqli $mysqli, string $col, string $value) : string {
    return sqlName($col) . '=' . sqlString($mysqli,$value);
}

function dbWereInt(  string $col, int $value) : string {
    return sqlName($col) . '=' . $value;
}
